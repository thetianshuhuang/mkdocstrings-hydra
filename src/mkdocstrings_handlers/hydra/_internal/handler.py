# This module implements a handler for Hydra.
...
from mkdocstrings_handlers.hydra._internal.config import HydraConfig, HydraOptions
...
class HydraHandler(BaseHandler):
    """The Hydra handler class."""

    name: ClassVar[str] = "hydra"
    """The handler's name."""

    domain: ClassVar[str] = "hydra"
    ...
    def __init__(self, config: HydraConfig, base_dir: Path, **kwargs: Any) -> None:
    ...
    def get_options(self, local_options: Mapping[str, Any]) -> HandlerOptions:
        ...
        try:
            return HydraOptions.from_data(**options)
        except Exception as error:
            raise PluginError(f"Invalid options: {error}") from error

    def collect(self, identifier: str, options: HydraOptions) -> CollectorItem:  # noqa: ARG002
    ...
    def render(self, data: CollectorItem, options: HydraOptions) -> str:
...
def get_handler(
    ...
    **kwargs: Any,
    ...
    """Simply return an instance of `HydraHandler`.
    ...
    Returns:
        An instance of `HydraHandler`.
    """
    base_dir = Path(tool_config.config_file_path or "./mkdocs.yml").parent
    return HydraHandler(
        config=HydraConfig.from_data(**handler_config),
        base_dir=base_dir,
        **kwargs,
    )
