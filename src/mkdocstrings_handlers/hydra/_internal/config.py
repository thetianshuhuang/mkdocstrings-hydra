...
try:
    ...
    if sys.version_info < (3, 10):
        try:
            ...
            _logger.debug(
                ...
                "Deactivating Pydantic validation for Hydra handler options.",
            )
            raise
    ...
    _base_url = "https://mkdocstrings.github.io/mkdocstrings-hydra/usage"
...
# YORE: EOL 3.9: Replace `**_dataclass_options` with `frozen=True, kw_only=True` within line.
@dataclass(**_dataclass_options)
class HydraOptions(HydraInputOptions):  # type: ignore[override,unused-ignore]
...
# YORE: EOL 3.9: Replace `**_dataclass_options` with `frozen=True, kw_only=True` within line.
@dataclass(**_dataclass_options)
class HydraInputConfig:
    """Hydra handler configuration."""

    # We want to validate options early, so we load them as `HydraInputOptions`.
    options: Annotated[
        HydraInputOptions,
        _Field(description="Configuration options for collecting and rendering objects."),
    ] = field(default_factory=HydraInputOptions)
...
# YORE: EOL 3.9: Replace `**_dataclass_options` with `frozen=True, kw_only=True` within line.
@dataclass(**_dataclass_options)
class HydraConfig(HydraInputConfig):  # type: ignore[override,unused-ignore]
    """Hydra handler configuration."""
