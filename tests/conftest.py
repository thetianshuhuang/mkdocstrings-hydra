...
if TYPE_CHECKING:
    ...
    from mkdocstrings_handlers.hydra.handler import HydraHandler
...
def fixture_mkdocs_conf(request: pytest.FixtureRequest, tmp_path: Path) -> Iterator[config.Config]:
    ...
    conf_dict = {
        ...
        "plugins": [{"mkdocstrings": {"default_handler": "hydra"}}],
...
def fixture_handler(plugin: MkdocstringsPlugin, ext_markdown: Markdown) -> HydraHandler:
    ...
    Returns:
        A handler instance.
    """
    handler = plugin.handlers.get_handler("hydra")
