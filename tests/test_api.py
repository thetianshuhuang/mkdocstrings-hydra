...
from mkdocstrings_handlers import hydra
...
def _fixture_loader() -> griffe.GriffeLoader:
    ...
    loader.load("mkdocstrings_handlers.hydra")
...
def _fixture_internal_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["mkdocstrings_handlers.hydra._internal"]


@pytest.fixture(name="public_api", scope="module")
def _fixture_public_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["mkdocstrings_handlers.hydra"]
...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `mkdocstrings_handlers.hydra`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in hydra.__all__ or not hasattr(hydra, obj.name)
    ]
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("mkdocstrings_handlers.hydra")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and _module_or_child("mkdocstrings_handlers.hydra", item.name)
        ):
